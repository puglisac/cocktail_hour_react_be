...
class TestViewRoutesTestCase(TestCase):
    ...
    def test_login(self):
        with app.test_client() as client:
            ...
            resp = client.post(
                '/login', data={'username': 'testname', 'password':   'testuser'}, follow_redirects=True)
            html = resp.get_data(as_text=True)
            self.assertEqual(resp.status_code, 200)
    ...
    def test_signup(self):
        with app.test_client() as client:
            ...
            resp = client.post('/signup', data={'username': 'testname', 'email': 'testemail@test.com',
                               'password':   'newpassword', 'dob': '01/02/1999'},  follow_redirects=True)
            html = resp.get_data(as_text=True)
            self.assertEqual(resp.status_code, 200)
